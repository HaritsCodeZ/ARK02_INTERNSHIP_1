...
<!-- SECTION 1: HERO (LIGHT PREMIUM IMAGE CONFIGURATION) -->
<section class="relative min-h-[75vh] flex items-center justify-center pt-12 pb-24 px-4 overflow-hidden bg-[#fcfcfc]">

    <div class="absolute inset-0 z-0">
        <img src="{{ asset('images/membershipsection1.jpg') }}"
             alt="{{ __('ARKOD Smart Logistics Background') }}"
    ...
    <div class="w-full max-w-[1400px] mx-auto text-center relative z-10">

        <div class="inline-flex items-center space-x-2 bg-neutral-950 text-white rounded-full px-4 py-2 mb-6 shadow-sm">
            <span class="text-[10px] sm:text-xs font-bold tracking-[0.3em] uppercase text-gray-200">{{ __('ARKOD SMART LOGITECH SDN. BHD') }}</span>
        </div>

        <!-- FIX 1: Scaled down mobile text size slightly & enforced hyphens/break-words to handle ultra-long translation strings safely -->
        <h1 class="text-3xl sm:text-4xl md:text-7xl lg:text-8xl font-black uppercase tracking-tight text-neutral-950 mb-6 leading-[1.1] break-words hyphens-auto drop-shadow-[0_2px_10px_rgba(255,255,255,0.8)]">
            {{ __('Elevating Your') }} <br class="hidden md:block"/>
            <span class="bg-gradient-to-r from-neutral-950 via-[#C5A850] to-neutral-950 bg-clip-text text-transparent">
                {{ __('Logistics Journey') }}
            </span>
        </h1>

        <p class="text-neutral-900 max-w-4xl mx-auto text-sm md:text-xl tracking-wide leading-relaxed mb-12 font-bold drop-shadow-[0_1px_8px_rgba(255,255,255,0.9)]">
            {{ __('Rewarding your loyalty, transforming your operational efficiency. Earn points automatically on verified tasks and unlock unmatched VIP corporate privileges.') }}
        </p>

        <!-- FIX 2: Restructured layout container without accidental duplicates and cleaned alignment boundaries -->
        <div class="w-full max-w-4xl mx-auto bg-gradient-to-br from-neutral-900 via-neutral-950 to-neutral-900 border-2 border-[#C5A850]/50 rounded-2xl p-6 md:py-8 md:px-4 backdrop-blur-md shadow-[0_30px_60px_rgba(197,168,80,0.15)] relative overflow-hidden flex flex-col md:flex-row justify-between items-center divide-y md:divide-y-0 md:divide-x divide-[#C5A850]/20 gap-y-6 md:gap-y-0">
            ...
            <div class="text-center px-4 md:flex-1 relative z-10 w-full">
                <span class="block text-xs uppercase tracking-widest text-neutral-400 mb-2 font-extrabold">{{ __('Points Validity') }}</span>
                <span class="text-lg md:text-xl font-black text-white tracking-wider">{{ __('12 MONTHS') }}</span>
            </div>

            <!-- Conversion Rate Info -->
            <div class="text-center px-4 md:flex-1 relative z-10 w-full pt-6 md:pt-0">
                <span class="block text-xs uppercase tracking-widest text-[#C5A850] mb-2 font-extrabold tracking-[0.15em]">{{ __('Conversion Rate') }}</span>
                <span class="text-lg md:text-xl font-black text-white tracking-wider bg-gradient-to-r from-yellow-200 to-amber-400 bg-clip-text text-transparent drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)]">{{ __('RM 1.00 = 1 POINT') }}</span>
            </div>

            <!-- Tier Validity Info -->
            <div class="text-center px-4 md:flex-1 relative z-10 w-full pt-6 md:pt-0">
                <span class="block text-xs uppercase tracking-widest text-neutral-400 mb-2 font-extrabold">{{ __('Tier Validity') }}</span>
                <span class="text-lg md:text-xl font-black text-white tracking-wider">{{ __('12 MONTHS') }}</span>
            </div>
        </div>
...
<div class="w-full bg-[#fcfcfc]">
    <section class="relative max-w-[1600px] mx-auto px-6 py-20 z-20" x-data="{ selectedTier: 'silver' }">
        ...
        <div class="text-center mb-16">
            <h2 class="text-xs font-black uppercase text-amber-600 tracking-[0.4em] mb-4">{{ __('Membership Tiers') }}</h2>
            <p class="text-3xl md:text-4xl font-black uppercase tracking-tight text-neutral-950">{{ __('Select a tier to preview privileges') }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch max-w-6xl mx-auto">
            ...
            <div @click="selectedTier = 'bronze'"
                ...
                <div>
                    <div class="flex justify-between items-start mb-10">
                        <div>
                            <span class="text-xs font-black tracking-[0.3em] uppercase text-amber-800 block mb-1">{{ __('Tier 01') }}</span>
                            <h3 class="text-3xl font-black uppercase tracking-wider text-amber-900">{{ __('Bronze') }}</h3>
                        </div>
                    ...
                    <div class="mb-8">
                        <span class="text-[10px] uppercase text-amber-800/80 block tracking-widest font-black mb-1">{{ __('Required Points') }}</span>
                        <div class="text-4xl font-black tracking-tight text-amber-950 mb-1 flex items-baseline gap-1">
                            <span>10,000</span>
                            <span class="text-xs uppercase font-black text-amber-800/60">{{ __('Pts') }}</span>
                        </div>
                        <span class="text-xs font-black text-amber-700 tracking-wide block">{{ __('Spending Matrix: RM10,000') }}</span>
                    </div>

                    <p class="text-sm text-amber-950 leading-relaxed mb-8 font-bold">
                        {{ __('Entry level membership designed for scaling businesses and regular clients starting their reward journey with ARKOD.') }}
                    </p>
                </div>
                ...
                <div class="border-t border-amber-200/80 pt-5 mt-auto">
                    <div class="flex items-center justify-between text-xs text-amber-900 font-black">
                        <span>{{ __('Standard Discount') }}</span>
                        <span class="text-white font-black bg-amber-700 px-3 py-1 rounded-md">{{ __('5% Off Base') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div @click="selectedTier = 'silver'"
                ...
                <div>
                    <div class="flex justify-between items-start mb-10">
                        <div>
                            <span class="text-xs font-black tracking-[0.3em] uppercase text-neutral-500 block mb-1">{{ __('Tier 02') }}</span>
                            <h3 class="text-3xl font-black uppercase tracking-wider text-neutral-800">{{ __('Silver') }}</h3>
                        </div>
                    ...
                    <div class="mb-8">
                        <span class="text-[10px] uppercase text-neutral-500 block tracking-widest font-black mb-1">{{ __('Required Points') }}</span>
                        <div class="text-4xl font-black tracking-tight text-neutral-900 mb-1 flex items-baseline gap-1">
                            <span>50,000</span>
                            <span class="text-xs uppercase font-black text-neutral-400">{{ __('Pts') }}</span>
                        </div>
                        <span class="text-xs font-black text-neutral-600 tracking-wide block">{{ __('Spending Matrix: RM50,000') }}</span>
                    </div>

                    <p class="text-sm text-neutral-700 leading-relaxed mb-8 font-black">
                        {{ __('Mid-tier status offering enhanced strategic priority values, broader reward limits, and dedicated campaign benefits.') }}
                    </p>
                </div>
                ...
                <div class="border-t border-neutral-300 pt-5 mt-auto">
                    <div class="flex items-center justify-between text-xs text-neutral-600 font-black">
                        <span>{{ __('Enhanced Privilege') }}</span>
                        <span class="text-white font-black bg-neutral-500 px-3 py-1 rounded-md">{{ __('10% Off + Priority') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div @click="selectedTier = 'gold'"
                ...
                <div>
                    <div class="flex justify-between items-start mb-10">
                        <div class="max-w-[75%]">
                            <span class="text-[10px] sm:text-xs font-black tracking-[0.25em] uppercase text-[#C5A850] block mb-1.5 break-words">{{ __('Tier 03 • Ultimate Premium') }}</span>
                            <h3 class="text-3xl font-black uppercase tracking-wider bg-gradient-to-r from-yellow-200 via-[#C5A850] to-yellow-100 bg-clip-text text-transparent">{{ __('Gold VIP') }}</h3>
                        </div>
                    ...
                    <div class="mb-8">
                        <span class="text-[10px] uppercase text-[#C5A850] block tracking-widest font-black mb-1">{{ __('Required Points') }}</span>
                        <div class="text-4xl sm:text-5xl font-black tracking-tight text-white mb-1 flex items-baseline gap-1">
                            <span>100,000</span>
                            <span class="text-xs uppercase font-black text-[#C5A850]">{{ __('Pts') }}</span>
                        </div>
                        <span class="text-xs font-black text-yellow-400 tracking-wide block">{{ __('Spending Matrix: RM100,000') }}</span>
                    </div>

                    <p class="text-sm text-neutral-200 leading-relaxed mb-8 font-black">
                        {{ __('The highest echelon of the ARKOD ecosystem. Reserved for top-value high loyalty corporate operations needing instantaneous logistical executions.') }}
                    </p>
                </div>
                ...
                <div class="border-t border-neutral-800 pt-5 mt-auto">
                    <div class="flex items-center justify-between text-xs text-neutral-300 font-black">
                        <span class="text-[#C5A850] uppercase tracking-widest text-[10px] font-black">{{ __('Elite Tier Rewards') }}</span>
                        <span class="text-white bg-gradient-to-r from-amber-500 to-yellow-500 font-black px-4 py-1.5 rounded-full text-[11px] shadow-sm shrink-0">{{ __('15% - 20% OFF') }}</span>
                    </div>
                </div>
            </div>
   ...
   <section class="w-full bg-[#fcfcfc] border-t border-b border-neutral-200/60 py-28 px-6 relative overflow-hidden"
         x-data="{
            ...
            privileges: [
                {
                    id: '01',
                    title: `{{ __('Special Discounts') }}`,
                    desc: `{{ __('Enjoy structural savings applied directly on contracts and continuous member-exclusive rates.') }}`,
                ...
                {
                    id: '02',
                    title: `{{ __('Priority Service') }}`,
                    desc: `{{ __('Faster structural processing response window and rapid priority line booking execution arrays.') }}`,
                ...
                {
                    id: '03',
                    title: `{{ __('Exclusive Promos') }}`,
                    desc: `{{ __('Access unique logistical structural pipelines and closed beta route optimizations updates.') }}`,
                ...
                {
                    id: '04',
                    title: `{{ __('Gold Account Manager') }}`,
                    desc: `{{ __('Personalized technical asset routing operations and direct support lines assigned specifically for you.') }}`,
    ...
    <div class="max-w-[1400px] mx-auto mb-16">
        <div class="max-w-3xl mx-auto text-center">
            <span class="text-xs font-black uppercase text-amber-600 tracking-[0.4em] block mb-3">{{ __('Core Privileges') }}</span>
            <h2 class="text-3xl md:text-5xl font-black uppercase tracking-wide text-neutral-950">{{ __('Ecosystem Capabilities') }}</h2>
        </div>
    </div>
    ...
    <div class="max-w-[1400px] mx-auto bg-gradient-to-br from-neutral-900 via-[#141414] to-neutral-950 border border-neutral-800/60 rounded-[40px] p-8 md:p-16 relative overflow-hidden shadow-[0_40px_100px_-15px_rgba(0,0,0,0.6)]">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
            ...
            <div class="lg:col-span-5 relative">
                <span class="text-xs font-black tracking-[0.4em] uppercase text-yellow-400/90 block mb-4 drop-shadow-sm">{{ __('Elite Segment Focus') }}</span>
                <h2 class="text-4xl md:text-5xl font-black uppercase tracking-tight mb-6 leading-[1.15] text-neutral-50">
                    {{ __('What Makes') }} <br/> <span class="bg-gradient-to-r from-yellow-400 via-amber-300 to-yellow-500 bg-clip-text text-transparent drop-shadow-sm">{{ __('Gold VIP') }}</span> {{ __('Special?') }}
                </h2>
                <p class="text-neutral-400 text-sm md:text-base leading-relaxed mb-8 font-medium max-w-md">
                    {{ __("We don't just upgrade your status; we re-engineer our corporate operation around your freight requirements. Gold tier turns standard interaction into high-speed automation.") }}
                </p>
                ...
                <div class="inline-flex items-center gap-3 bg-gradient-to-r from-yellow-400 to-amber-400 text-black font-black text-xs md:text-sm uppercase tracking-widest px-8 py-4 rounded-xl shadow-[0_10px_30px_-5px_rgba(234,179,8,0.3)] transition-all duration-300 hover:scale-[1.03] hover:shadow-[0_15px_35px_-3px_rgba(234,179,8,0.4)] cursor-pointer group">
                    <span>{{ __('Qualify At 100,000 Points') }}</span>
            ...
            <div class="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="group bg-neutral-900/50 hover:bg-neutral-900/90 border border-white/5 hover:border-yellow-400/30 rounded-2xl p-7 backdrop-blur-md shadow-xl transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.7)] flex flex-col justify-between">
                    <div>
                        ...
                        <h4 class="text-neutral-100 group-hover:text-yellow-400 font-black uppercase tracking-wide mb-2 text-sm md:text-base transition-colors duration-300">{{ __('Dedicated Account PIC') }}</h4>
                        <p class="text-neutral-400 group-hover:text-neutral-300 text-xs md:text-sm leading-relaxed font-medium transition-colors duration-300">{{ __('One professional team member manages your system assets entirely. No general automated help desks.') }}</p>
                    </div>
                </div>
                ...
                <div class="group bg-neutral-900/50 hover:bg-neutral-900/90 border border-white/5 hover:border-yellow-400/30 rounded-2xl p-7 backdrop-blur-md shadow-xl transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.7)] flex flex-col justify-between">
                    <div>
                        <div class="text-3xl mb-4 filter drop-shadow transition-transform duration-500 group-hover:scale-110 origin-left">⚡</div>
                        <h4 class="text-neutral-100 group-hover:text-yellow-400 font-black uppercase tracking-wide mb-2 text-sm md:text-base transition-colors duration-300">{{ __('Instant Reply Threshold') }}</h4>
                        <p class="text-neutral-400 group-hover:text-neutral-300 text-xs md:text-sm leading-relaxed font-medium transition-colors duration-300">{{ __('Priority replies enforced inside corporate operational limits. Expect resolution arrays inside the exact same business day.') }}</p>
                    </div>
                </div>
                ...
                <div class="group bg-neutral-900/50 hover:bg-neutral-900/90 border border-white/5 hover:border-yellow-400/30 rounded-2xl p-7 backdrop-blur-md shadow-xl transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.7)] flex flex-col justify-between">
                    <div>
                        <div class="text-3xl mb-4 filter drop-shadow transition-transform duration-500 group-hover:scale-110 origin-left">💬</div>
                        <h4 class="text-neutral-100 group-hover:text-yellow-400 font-black uppercase tracking-wide mb-2 text-sm md:text-base transition-colors duration-300">{{ __('Direct WhatsApp Comms') }}</h4>
                        <p class="text-neutral-400 group-hover:text-neutral-300 text-xs md:text-sm leading-relaxed font-medium transition-colors duration-300">{{ __('Direct communication channel configurations instead of traditional ticketing networks or switchboards.') }}</p>
                    </div>
                </div>
                ...
                <div class="group bg-neutral-900/50 hover:bg-neutral-900/90 border border-white/5 hover:border-yellow-400/30 rounded-2xl p-7 backdrop-blur-md shadow-xl transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.7)] flex flex-col justify-between">
                    <div>
                        <div class="text-3xl mb-4 filter drop-shadow transition-transform duration-500 group-hover:scale-110 origin-left">🚀</div>
                        <h4 class="text-neutral-100 group-hover:text-yellow-400 font-black uppercase tracking-wide mb-2 text-sm md:text-base transition-colors duration-300">{{ __('Priority Logistics Processing') }}</h4>
                        <p class="text-neutral-400 group-hover:text-neutral-300 text-xs md:text-sm leading-relaxed font-medium transition-colors duration-300">{{ __('Your manifests, updates, customs declarations, and custom packaging request queues processed first.') }}</p>
                    </div>
                </div>
...
<section class="bg-white py-32 px-6 border-t border-neutral-200 relative font-sans selection:bg-amber-100 selection:text-neutral-900">
    <div class="max-w-[1400px] mx-auto">
        ...
        <div class="text-center max-w-3xl mx-auto mb-24">
            <span class="text-xs font-black uppercase text-amber-600 tracking-[0.5em] block mb-4">{{ __('System Specifications') }}</span>
            <h2 class="text-4xl md:text-5xl font-black uppercase tracking-tight text-neutral-950 leading-tight">
                {{ __('Ecosystem Management') }} <br class="hidden sm:inline"/> {{ __('& Maintenance') }}
            </h2>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 xl:gap-12">
            ...
            <div class="group bg-[#0d0d0d] border-t-4 border-t-amber-500 border-x border-b border-neutral-900 rounded-2xl p-8 md:p-10 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_40px_80px_-15px_rgba(217,119,6,0.2)] flex flex-col justify-between items-center text-center">
                <div>
                    ...
                    <div class="flex items-center justify-center gap-3 mb-6">
                        ...
                        <span class="text-[10px] font-black uppercase tracking-[0.3em] text-amber-500/80">{{ __('Protocol 01') }}</span>
                    </div>
                    ...
                    <p class="text-neutral-300 text-base md:text-lg leading-relaxed font-medium mb-8 max-w-sm mx-auto">
                        {{ __('Membership structures are analyzed every 12 months from qualification dates. Notification pathways update profiles via dashboards in the first 6 months as preventative follow-up metrics.') }}
                    </p>
                </div>
            ...
            <div class="group bg-[#0d0d0d] border-t-4 border-t-amber-500 border-x border-b border-neutral-900 rounded-2xl p-8 md:p-10 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_40px_80px_-15px_rgba(217,119,6,0.2)] flex flex-col justify-between items-center text-center">
                <div class="w-full">
                    ...
                    <div class="flex items-center justify-center gap-3 mb-6">
                        <span class="w-2.5 h-2.5 rounded-full bg-amber-400 group-hover:scale-125 transition-transform duration-300 shadow-[0_0_10px_#fbbf24]"></span>
                        <span class="text-[10px] font-black uppercase tracking-[0.3em] text-amber-500/80">{{ __('Protocol 02') }}</span>
                    </div>

                    <h3 class="text-xl md:text-2xl font-black uppercase tracking-wide mb-5 text-white group-hover:text-amber-400 transition-colors duration-300">
                        {{ __('Point Recovery Matrix') }}
                    </h3>

                    <!-- Significantly Enlarged & Center-Aligned Paragraph -->
                    <p class="text-neutral-300 text-base md:text-lg leading-relaxed font-medium mb-6 max-w-sm mx-auto">
                        {{ __('Points are bound to transaction accounts for exactly 12 months. Upon expiration, structures enter auto-freeze pipelines. Recovery operations invoke tier maintenance fees:') }}
                    </p>
                </div>

                <!-- Premium Dark Ledgers Table -->
            ...
            <div class="group bg-[#0d0d0d] border-t-4 border-t-amber-500 border-x border-b border-neutral-900 rounded-2xl p-8 md:p-10 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_40px_80px_-15px_rgba(217,119,6,0.2)] flex flex-col justify-between items-center text-center">
                <div>
                    ...
                    <div class="flex items-center justify-center gap-3 mb-6">
                        <span class="w-2.5 h-2.5 rounded-full bg-amber-400 group-hover:scale-125 transition-transform duration-300 shadow-[0_0_10px_#fbbf24]"></span>
                        <span class="text-[10px] font-black uppercase tracking-[0.3em] text-amber-500/80">{{ __('Protocol 03') }}</span>
                    </div>

                    <h3 class="text-xl md:text-2xl font-black uppercase tracking-wide mb-5 text-white group-hover:text-amber-400 transition-colors duration-300">
                        {{ __('Downgrade Parameters') }}
                    </h3>

                    <!-- Significantly Enlarged & Center-Aligned Paragraph -->
                    <p class="text-neutral-300 text-base md:text-lg leading-relaxed font-medium mb-8 max-w-sm mx-auto">
                        {{ __('Failure to sustain structural points targets inside the 12-month window initiates downgrade cascades: Gold profiles step down to Silver/Bronze levels, and Silver statuses revert to Bronze parameters.') }}
                    </p>
                </div>
                ...
                <div>
                    ...
                    <span class="inline-block text-xs font-black uppercase tracking-widest text-red-400 bg-red-950/40 border border-red-900/60 px-5 py-2 rounded-xl shadow-sm">
                        {{ __('Status Protection Laws Apply') }}
                    </span>
                </div>
            </div>
...
<!-- Final Section Wrapper: Full-Bleed Clean White Background -->
<section class="w-full bg-white py-12 sm:py-36 px-4 sm:px-6 relative z-10 font-sans text-center selection:bg-amber-400 selection:text-black">
    <div class="max-w-[1400px] mx-auto">

        <!-- Trust Badge Element -->
        <div class="mb-8 flex justify-center">
            <!-- Adjusted tracking and font weight slightly for better small-screen fit -->
            <span class="inline-flex items-center gap-2.5 text-[10px] sm:text-xs font-black tracking-[0.25em] sm:tracking-[0.4em] uppercase text-neutral-900 bg-neutral-100 border border-neutral-200/60 px-4 sm:px-5 py-2.5 rounded-full shadow-sm max-w-full">
                <span class="w-2 h-2 rounded-full bg-amber-500 animate-pulse shrink-0"></span>
                <span class="truncate">{{ __('ARKOD Global Infrastructure') }}</span>
            </span>
        </div>

        <!-- Massive Full-Scale Editorial Headline -->
        <!-- FIX 1: Reduced base mobile size to text-3xl/text-4xl and added break-words hyphens-auto to protect container layout -->
        <h3 class="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-black uppercase tracking-tight text-neutral-950 mb-8 leading-[1.1] max-w-5xl mx-auto break-words hyphens-auto">
            {{ __('Ready to Optimize Your') }} <br class="hidden sm:inline" /> <span class="text-amber-500">{{ __('Logistics Value?') }}</span>
        </h3>

        <!-- Large, Highly Legible Description Block -->
        <!-- FIX 2: Dropped base text size to text-sm on tiny screens to improve long paragraph wrapping flow -->
        <p class="text-neutral-600 text-sm sm:text-base md:text-xl font-medium leading-relaxed mb-14 max-w-3xl mx-auto">
            {{ __('Join thousands of high-velocity enterprises tracking global milestones via the ARKOD infrastructure ecosystem. Start your account profile initialization process today.') }}
        </p>

        <!-- Ultra-Premium High-Contrast Button Station -->
        <div class="flex justify-center items-center px-2">
            <!-- FIX 3: Reduced mobile padding to px-6 (scales back up to px-12 on sm screens) to allow long text strings space inside the button asset safely -->
            <a href="#" class="group inline-flex items-center gap-3 sm:gap-4 bg-gradient-to-r from-yellow-400 via-amber-400 to-yellow-500 text-black font-black px-6 sm:px-12 py-4 sm:py-5 rounded-2xl text-xs sm:text-sm md:text-base uppercase tracking-widest transition-all duration-300 transform hover:scale-[1.04] hover:shadow-[0_25px_50px_-10px_rgba(234,179,8,0.5)] shadow-[0_15px_30px_-10px_rgba(217,119,6,0.3)] w-full sm:w-auto justify-center break-words">
                <span>{{ __('SIGN UP AS A MEMBER NOW!') }}</span>
                <!-- High-Definition Directional Vector -->
                <svg class="w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 transform group-hover:translate-x-1.5 shrink-0" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24">

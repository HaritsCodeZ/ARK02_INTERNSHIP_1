...
<section class="relative min-h-[85vh] flex items-center justify-center px-6 py-24 text-center overflow-hidden bg-black">
    ...
    <div class="absolute inset-0 z-0">
        ...
        <img src="{{ asset('images/specialpackagesentry.png') }}"
             alt="{{ __('ARKOD Special Packages Premium Workshop Entry') }}"
    ...
    <div class="max-w-[1200px] mx-auto relative z-30 space-y-8 flex flex-col items-center">
        ...
        <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-yellow-500/40 bg-black/80 backdrop-blur-md shadow-xl text-[10px] font-black uppercase tracking-[0.35em]">
            ...
            <span class="text-white">{{ __('Exclusive Transit Upgrades') }} <br> {{ __('SPECIFIC FOR VEHICHLES') }}</span>
            <span class="w-1.5 h-1.5 rounded-full bg-yellow-500"></span>
        </div>
        ...
        <h1 class="text-4xl md:text-7xl font-black uppercase tracking-tight leading-[1.05] text-white">
            {{ __('ELITE CARE FOR') }} <br>
            <span class="text-[#facc15]">{{ __('YOUR VEHICLE') }}</span>
        </h1>
        ...
        <div class="max-w-3xl mx-auto bg-black/50 backdrop-blur-md px-8 py-6 rounded-2xl border border-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
            <p class="text-gray-300 text-sm md:text-base md:leading-relaxed font-medium tracking-wide">
                {{ __('Don’t just ship your ride—preserve its status. Upgrade your standard vehicle logistics package with premium detailing and engineering maintenance bundles tailored for long-distance maritime transit.') }}
            </p>
        </div>

        <!-- Minimalist Flow Indicator Component -->
        <div class="pt-4 flex flex-col items-center gap-2.5 group cursor-pointer">
            <span class="text-[9px] uppercase tracking-[0.5em] text-gray-400 font-black transition-colors group-hover:text-yellow-400">{{ __('Discover Packages') }}</span>
...
<section class="py-24 bg-white px-6 relative text-white">
     <!-- Header -->
        <div class="text-center mb-16">
            <h2 class="text-5xl font-black text-[#0a0a0a] tracking-tighter">{{ __('DESIRE-MATCHED PACKAGES') }}</h2>
            <div class="w-24 h-1 bg-yellow-400 mx-auto mt-4 mb-6"></div>
            <p class="text-gray-600 text-lg max-w-2xl mx-auto">
                {{ __('Here at Arkod Smart Logitech we provide the best for our clients') }}
            </p>
        </div>
    <div class="max-w-[1600px] mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch">
            ...
            <div class="group relative flex flex-col justify-between bg-neutral-950 border border-neutral-800 rounded-3xl p-8 shadow-2xl transition-all duration-500 hover:border-amber-500/50 hover:shadow-amber-900/10">
                <div>
                    <div class="w-full h-56 bg-black rounded-2xl overflow-hidden mb-8 relative border border-neutral-800">
                        <img src="{{ asset('images/CAR SHIPPING PACKAGE.png') }}" alt="{{ __('Car Shipping Package') }}" class="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity duration-500">
                        <div class="absolute inset-0 bg-gradient-to-t from-neutral-950 via-transparent to-transparent"></div>
                        <span class="absolute bottom-4 left-4 text-[10px] font-black uppercase text-amber-400 tracking-[0.2em] bg-black/50 backdrop-blur px-3 py-1 rounded-full border border-amber-500/20">{{ __('Logistics Core') }}</span>
                    </div>

                    <h3 class="text-2xl font-black uppercase tracking-wider text-white mb-2">{{ __('Car Shipping') }} <br><span class="text-amber-500">{{ __('Package') }}</span></h3>
                    <p class="text-neutral-400 text-sm font-medium leading-relaxed mb-8">{{ __('The industry standard for secure cross-strait transit, ensuring your vehicle remains protected from port to port.') }}</p>

                    <div class="space-y-4 mb-8">
                        <div class="flex items-center gap-3 text-sm text-neutral-300"><span class="text-amber-500 font-bold">•</span> {{ __('Port Klang, Kuching, Bintulu, KK') }}</div>
                        <div class="flex items-center gap-3 text-sm text-neutral-300"><span class="text-amber-500 font-bold">•</span> {{ __('Real-time asset tracking') }}</div>
                    </div>
                </div>

                <div class="pt-6 border-t border-neutral-800">
                    <p class="text-[10px] uppercase tracking-[0.2em] text-neutral-500 font-bold italic">"{{ __('Reliability defines the foundation of every transit.') }}"</p>
                </div>
            </div>
            ...
            <div class="group relative flex flex-col justify-between bg-neutral-950 border border-neutral-800 rounded-3xl p-8 shadow-2xl transition-all duration-500 hover:border-amber-500/50 hover:shadow-amber-900/10">
                <div>
                    <div class="w-full h-56 bg-black rounded-2xl overflow-hidden mb-8 relative border border-neutral-800">
                        <img src="{{ asset('images/LIQUIMOLY_OIL.png') }}" alt="{{ __('Essentials Package') }}" class="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity duration-500">
                        <div class="absolute inset-0 bg-gradient-to-t from-neutral-950 via-transparent to-transparent"></div>
                        <span class="absolute bottom-4 left-4 text-[10px] font-black uppercase text-amber-400 tracking-[0.2em] bg-black/50 backdrop-blur px-3 py-1 rounded-full border border-amber-500/20">{{ __('Performance Core') }}</span>
                    </div>

                    <h3 class="text-2xl font-black uppercase tracking-wider text-white mb-2">{{ __('Essentials') }} <br><span class="text-amber-500">{{ __('Package') }}</span></h3>
                    <p class="text-neutral-400 text-sm font-medium leading-relaxed mb-8">{{ __("Elevate your engine's health with professional maintenance, utilizing Liqui Moly performance fluids for long-distance durability .") }}</p>

                    <div class="space-y-4 mb-8">
                        <div class="flex items-center gap-3 text-sm text-neutral-300"><span class="text-amber-500 font-bold">•</span> {{ __('Liqui Moly 5W-30 Special Tec T') }}</div>
                        <div class="flex items-center gap-3 text-sm text-neutral-300"><span class="text-amber-500 font-bold">•</span> {{ __('H.B Auto mechanical evaluation') }}</div>
                    </div>
                </div>

                <div class="pt-6 border-t border-neutral-800">
                    <p class="text-[10px] uppercase tracking-[0.2em] text-neutral-500 font-bold italic">"{{ __('Precision maintenance for peak performance.') }}"</p>
                </div>
            </div>
            ...
            <div class="group relative flex flex-col justify-between bg-neutral-950 border border-neutral-800 rounded-3xl p-8 shadow-2xl transition-all duration-500 hover:border-amber-500/50 hover:shadow-amber-900/10">
                <div>
                    <div class="w-full h-56 bg-black rounded-2xl overflow-hidden mb-8 relative border border-neutral-800">
                        <img src="{{ asset('images/AESTHETICPACKAGE.png') }}" alt="{{ __('Aesthetic Package') }}" class="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity duration-500">
                        <div class="absolute inset-0 bg-gradient-to-t from-neutral-950 via-transparent to-transparent"></div>
                        <span class="absolute bottom-4 left-4 text-[10px] font-black uppercase text-amber-400 tracking-[0.2em] bg-black/50 backdrop-blur px-3 py-1 rounded-full border border-amber-500/20">{{ __('Showroom Core') }}</span>
                    </div>

                    <h3 class="text-2xl font-black uppercase tracking-wider text-white mb-2">{{ __('Aesthetic') }} <br><span class="text-amber-500">{{ __('Package') }}</span></h3>
                    <p class="text-neutral-400 text-sm font-medium leading-relaxed mb-8">{{ __('Achieve the ultimate showroom finish with our multi-tier protective coating and deep-clean grooming services.') }}</p>

                    <div class="space-y-4 mb-8">
                        <div class="flex items-center gap-3 text-sm text-neutral-300"><span class="text-amber-500 font-bold">•</span> {{ __('GYEON & KCX Protection systems') }}</div>
                        <div class="flex items-center gap-3 text-sm text-neutral-300"><span class="text-amber-500 font-bold">•</span> {{ __('Hydrophobic deep engine bay clean') }}</div>
                    </div>
                </div>

                <div class="pt-6 border-t border-neutral-800">
                    <p class="text-[10px] uppercase tracking-[0.2em] text-neutral-500 font-bold italic">"{{ __('Excellence is in the finest details.') }}"</p>
                </div>
            </div>
...
<section class="py-24 bg-white text-black border-t border-neutral-100 px-6 relative overflow-hidden">
    ...
    <div class="max-w-[1200px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
        ...
        <div class="lg:col-span-7 space-y-6 text-left">
            <span class="text-xs font-black tracking-[0.4em] uppercase text-amber-600 block">{{ __('Seamless Experience') }}</span>
            <h2 class="text-3xl md:text-5xl font-black uppercase tracking-tight text-black leading-tight">
                {{ __('UNCOMPROMISING TRANSIT') }} <br>{{ __('FOR REFINED OWNERS') }}
            </h2>
            <div class="w-12 h-[3px] bg-amber-500"></div>
            <p class="text-neutral-600 text-sm md:text-base leading-relaxed max-w-xl font-medium">
                {{ __('Join thousands of smart vehicle owners who trust Arkod Smart Logitech to handle cross-strait vehicle transport with unparalleled premium attention. Lock in your custom selection during checkout for premium treatment.') }}
            </p>
        ...
        <div class="lg:col-span-5 relative">
            <div class="w-full h-80 bg-neutral-100 border border-neutral-200 rounded-2xl overflow-hidden shadow-lg relative group">
                ...
                <div class="absolute bottom-6 left-6 right-6 flex justify-between items-center text-white">
                    <div>
                        <p class="text-[9px] font-bold text-amber-400 uppercase tracking-widest">{{ __('Official Portal') }}</p>
                        <p class="text-sm font-black uppercase tracking-wider">{{ __('arkod.com.my') }}</p>
                    </div>
                    <span class="text-[10px] bg-white/10 backdrop-blur-md px-3 py-1 rounded border border-white/20 font-bold uppercase tracking-wider">{{ __('T&Cs Applied') }}</span>
                </div>
            </div>
        </div>

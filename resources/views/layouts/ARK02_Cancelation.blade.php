...
<head>
    ...
    <title>ARKOD | Cancellation Policies</title>
...
<body class="bg-[#0a0a0a] font-montserrat antialiased text-white min-h-screen flex flex-col justify-between">
    <div>
        ...
        <div class="space-y-6 mt-6 max-w-[1600px] mx-auto px-6 mb-12">
            ...
            <div x-data="{ open: true }" class="bg-[#0f0f0f] border border-white/10 rounded-lg overflow-hidden transition-all duration-300 hover:border-yellow-400/30">
                ...
                <div x-show="open" x-collapse class="p-6 pt-0 border-t border-white/5 text-gray-400 text-sm leading-relaxed space-y-4">
                    <p>Collection failures may occasionally occur if a courier agent is unable to locate the specified address, experiences route disruptions, or encounters unexpected traffic bottlenecks. Shippers retain the right to request a collection date adjustment. It remains the user's responsibility to verify address accuracy and ensure a physical proxy is on-site during the newly scheduled window to guarantee timely execution.</p>

                    <p>Alternatively, users can invoke the following resolution routes if a pickup falls through:</p>

                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 my-4">
                        <div class="bg-black/50 p-4 rounded border border-white/5 hover:border-yellow-400/20 transition group">
                            <span class="text-yellow-400 font-bold block uppercase tracking-wider text-xs mb-1 group-hover:text-white transition-colors">01. Remind Courier</span>
                            <p class="text-xs text-gray-400">Issue an immediate system ping prioritizing your parcel for the earliest available pickup slot.</p>
                        </div>
                        <div class="bg-black/50 p-4 rounded border border-white/5 hover:border-yellow-400/20 transition group">
                            <span class="text-yellow-400 font-bold block uppercase tracking-wider text-xs mb-1 group-hover:text-white transition-colors">02. Rebook Route</span>
                            <p class="text-xs text-gray-400">This secondary routing parameter unlocks automatically on your dashboard following a second consecutive missed collection.</p>
                        </div>
                        <div class="bg-black/50 p-4 rounded border border-white/5 hover:border-yellow-400/20 transition group">
                            <span class="text-red-400 font-bold block uppercase tracking-wider text-xs mb-1 group-hover:text-red-300 transition-colors">03. Void Consignment</span>
                            <p class="text-xs text-gray-400">Cancel the delivery assignment entirely. All processed transit fees will be safely reversed to your balance.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div x-data="{ open: true }" class="bg-[#0f0f0f] border border-white/10 rounded-lg overflow-hidden transition-all duration-300 hover:border-yellow-400/30">
                ...
                <div x-show="open" x-collapse class="p-6 pt-0 border-t border-white/5 text-gray-400 text-sm leading-relaxed space-y-4">
                    <div class="bg-white/5 p-4 rounded border border-white/10 text-xs space-y-2">
                        <span class="text-yellow-400 font-bold block uppercase tracking-wide">Permissible Order Status Matrix</span>
                        <p>Cancellations can be initialized natively if your shipment resides strictly under either of the following system lifecycle stages:</p>
                        <div class="flex flex-wrap gap-2 pt-1">
                            <span class="px-3 py-1 bg-yellow-400/10 text-yellow-400 font-mono font-bold rounded border border-yellow-400/20 uppercase tracking-wider text-[10px]">
                    ...
                    <div class="bg-black p-4 rounded border border-white/5 text-xs">
                        <span class="text-white font-bold block uppercase tracking-wider mb-2">How to Request a Cancellation:</span>
                        <p class="mb-3 text-gray-400">If your consignment meets the verification states listed above, initiate the support flow via email:</p>
                        <div class="inline-flex items-center space-x-3 bg-white/5 px-4 py-3 rounded border border-white/10">
                            <svg class="w-4 h-4 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002 -2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                            </svg>
                            <span class="text-gray-300">Contact Desk:</span>
                            <a href="mailto:support@arkod.com.my" class="text-yellow-400 hover:text-white font-mono font-bold underline transition-colors">
                                support@arkod.com.my
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <footer class="bg-black border-t border-white/10 px-6 py-8 w-full mt-auto">

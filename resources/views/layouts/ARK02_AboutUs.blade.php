...
<!-- START: PROVEN LOGISTICS SECTION -->
<section class="relative min-h-[500px] lg:h-[600px] flex items-center overflow-hidden" x-data="heroSlider()">
        ...
        <div class="max-w-4xl text-right md:text-left lg:ml-auto">
            ...
            <template x-for="(slide, index) in slides" :key="index">
                <div x-show="current === index"
                    ...
                    <h2 class="text-4xl md:text-5xl font-black tracking-tighter leading-tight mb-4 text-white">
            ...
            <!-- REFACTORED KPI SECTION BELOW -->
            <template x-for="(slide, index) in slides" :key="index">
                <div x-show="current === index" class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                    <template x-for="(kpi, i) in slide.kpis" :key="i">
                        <!-- Left-aligned Row layout on mobile, snaps back to original layout via md: breakpoint -->
                        <div class="flex items-center md:flex-col md:items-start gap-4 md:gap-3">
                            <div class="p-2 bg-yellow-400/10 border border-yellow-400/20 rounded-lg shrink-0">
                                <span x-html="kpi.icon"></span>
                            </div>
                            <div class="text-left">
                                <!-- Enforces line-height separation to prevent vertical overlapping text on tight views -->
                                <div class="text-xl md:text-2xl font-black text-white leading-none tracking-wide" x-text="kpi.number"></div>
                                <div class="text-[11px] font-bold text-yellow-400 uppercase tracking-widest mt-1.5 leading-tight" x-text="kpi.label"></div>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <div class="flex space-x-2 justify-end md:justify-start">
                <template x-for="(slide, index) in slides" :key="index">
                    <button @click="goTo(index)"
                            ...
                            class="h-1 rounded-full transition-all duration-300 hover:bg-yellow-400 focus:outline-none"></button>
                </template>
            </div>
        </div>
    </div>
</section>
<!-- END: PROVEN LOGISTICS SECTION -->
...
<section class="py-32 bg-[#fcfcfc] font-montserrat overflow-hidden relative"
    ...
    <div class="absolute top-0 left-0 w-full h-full pointer-events-none opacity-[0.03] select-none z-0">
        <div class="absolute top-20 -left-20 text-[20vw] font-black leading-none uppercase rotate-90">{{ __('LOGISTICS') }}</div>
        <div class="absolute bottom-0 -right-20 text-[20vw] font-black leading-none uppercase -rotate-90">{{ __('SUCCESS') }}</div>
    </div>
    ...
    <div class="max-w-[1400px] mx-auto px-6 relative z-10">
        ...
        <div class="text-center mb-32 relative" x-data="{ shown: false }" x-intersect.once="shown = true">
            <div class="inline-flex items-center gap-4 mb-6" x-show="shown" x-transition:enter="transition ease-out duration-700">
                ...
                <span class="text-yellow-600 font-black text-xs uppercase tracking-[0.6em]">{{ __('Portfolio') }}</span>
                <span class="h-[1px] w-12 bg-yellow-400"></span>
            </div>
            ...
            <p class="text-black text-base md:text-lg max-w-2xl mx-auto font-medium leading-relaxed"
               x-show="shown" x-transition:enter="transition ease-out duration-1000 delay-300">
                {{ __('Every shipment managed highlights our reliability and unwavering commitment to client success.') }}
            </p>
        </div>

       <div class="relative space-y-32">

    <div class="group flex flex-col lg:flex-row items-center gap-6 lg:gap-24 relative"
     x-data="{ active: false }"
     @mouseenter="active = true"
     @mouseleave="active = false">

    <div class="hidden lg:block absolute -top-10 -left-55 text-[12rem] font-black text-black/5 leading-none z-0 pointer-events-none transition-all duration-700"
         :class="active ? 'translate-x-3' : ''">
        01
    </div>

    <div class="w-full lg:w-3/5 relative z-10">
        <div class="relative p-0.5 lg:p-2 border border-black/5 rounded-2xl lg:rounded-none lg:rounded-tl-[120px] bg-white shadow-md lg:shadow-sm transition-transform duration-500"
             :class="active ? 'lg:-translate-y-3' : ''">

            <div class="relative p-0 lg:p-4 lg:border-[16px] lg:border-yellow-400 lg:border-r-0 lg:border-b-0 lg:rounded-none lg:rounded-tl-[110px]">
                <div class="overflow-hidden rounded-xl lg:rounded-none lg:rounded-tl-[85px] shadow-lg lg:shadow-2xl relative group">
                    <img src="{{ asset('images/therealcontent1aboutpipes.jpeg') }}" alt="{{ __('The 3 Pipes') }}"
                         class="w-full aspect-[16/9] object-cover transition-transform duration-1000 group-hover:scale-105">
                </div>
            </div>
        </div>
    </div>

    <div class="w-full lg:w-2/5 py-2 lg:py-4 px-1">
        <div class="inline-block px-3 py-1.5 bg-yellow-400 text-black text-[10px] font-black uppercase tracking-widest mb-3 rounded">
            {{ __('Logistics Excellence') }}
        </div>

        <h3 class="text-xl sm:text-2xl lg:text-3xl font-black text-black leading-tight mb-3 transition-colors group-hover:text-yellow-600">
            {{ __('Dominating The Horizon: Engineering Beyond Boundaries') }}
        </h3>

        <p class="text-gray-600 text-sm sm:text-base leading-relaxed mb-5 opacity-90 border-l-4 border-yellow-400 pl-4">
            {{ __('From Manila to Kuching, we don’t just move cargo—we transport the massive infrastructure that shapes tomorrow.') }}
        </p>

        <button @click="modal1Open = true; document.body.style.overflow = 'hidden'"
                class="group/btn inline-flex items-center gap-3 text-black font-black text-xs sm:text-sm uppercase tracking-widest cursor-pointer focus:outline-none">
    ...
    <!-- STORY 02 -->
<div class="group flex flex-col lg:flex-row-reverse items-center gap-6 lg:gap-24 relative"
     x-data="{ active: false }"
     @mouseenter="active = true"
     @mouseleave="active = false">

    <!-- DELETED ON MOBILE: Giant absolute "02" number hidden on mobile to prevent layout breaking -->
    <div class="hidden lg:block absolute -top-10 -right-55 text-[12rem] font-black text-black/[0.25] leading-none z-0 transition-all duration-700"
    ...
    <!-- IMAGE WRAPPER -->
    <div class="w-full lg:w-3/5 relative z-10">
        <!-- Clean rounded corners for mobile; premium asymmetric look preserved on desktop via lg: -->
        <div class="relative p-0.5 lg:p-2 border border-black/5 rounded-2xl lg:rounded-none lg:rounded-tr-[120px] lg:rounded-br-[120px] bg-white shadow-md lg:shadow-sm transition-transform duration-500"
             :class="active ? 'lg:-translate-y-3' : ''">

            <!-- DELETED DECORATIVE YELLOW BORDER SPACE ON MOBILE: Frees up image space on narrow phone screens -->
            <div class="relative p-0 lg:p-4 lg:border-[16px] lg:border-yellow-400 lg:border-l-0 lg:rounded-none lg:rounded-tr-[110px] lg:rounded-br-[110px]">
                <div class="overflow-hidden rounded-xl lg:rounded-none lg:rounded-tr-[85px] lg:rounded-br-[85px] shadow-lg lg:shadow-2xl relative group">
                    <img src="{{ asset('images/therealcontent2aboutcar.png') }}" alt="{{ __('Car Shipping') }}"
                         class="w-full aspect-[16/9] object-cover transition-transform duration-1000 group-hover:scale-105">
                </div>
            </div>
        </div>
    ...
    <div class="w-full lg:w-2/5 py-2 lg:py-8 px-1">
        <!-- Upgraded badge background & text padding contrast for visual pop -->
        <div class="inline-block px-3 py-1.5 bg-yellow-400 text-black text-[10px] font-black uppercase tracking-widest mb-3 rounded">
            {{ __('Specialized Transport') }}
        </div>

        <!-- Optimized font size for mobile text scaling -->
        <h3 class="text-xl sm:text-2xl lg:text-3xl font-black text-black leading-tight mb-3 group-hover:text-yellow-600 transition-colors">
            {{ __('Bridging Oceans') }}, {{ __('Securing Valuables') }}
        </h3>

        <!-- Fixed border layout: Clean left-aligned accent border for perfect text flow on mobile and desktop -->
        <p class="text-gray-600 text-sm sm:text-base leading-relaxed mb-5 opacity-90 border-l-4 border-yellow-400 pl-4">
            {{ __('From the shores of Sarawak to the heart of the capital, Arkod Smart Logitech specializes in handling high-value automotive transportation and specialized mixed-cargo consolidations.') }}
        </p>
...
<div class="group flex flex-col lg:flex-row items-center gap-6 lg:gap-24 relative"
    ...
    <!-- DELETED ON MOBILE: Giant absolute "03" hidden on mobile to stop horizontal scroll bugs -->
    <div class="hidden lg:block absolute -top-10 -left-55 text-[12rem] font-black text-black/[0.25] leading-none z-0 transition-all duration-700"
         :class="active ? 'translate-x-5' : ''">
        03
    </div>
    ...
    <!-- IMAGE WRAPPER -->
    <div class="w-full lg:w-3/5 relative z-10">
        <!-- Clean, unified corners for mobile; premium asymmetric look preserved on desktop via lg: -->
        <div class="relative p-0.5 lg:p-2 border border-black/5 rounded-2xl lg:rounded-none lg:rounded-tl-[120px] bg-white shadow-md lg:shadow-sm transition-transform duration-500"
             :class="active ? 'lg:-translate-y-3' : ''">

            <!-- DELETED DECORATIVE YELLOW BORDER SPACE ON MOBILE: Keeps image max width on small phones -->
            <div class="relative p-0 lg:p-4 lg:border-[16px] lg:border-yellow-400 lg:border-r-0 lg:border-b-0 lg:rounded-none lg:rounded-tl-[110px]">
                <div class="overflow-hidden rounded-xl lg:rounded-none lg:rounded-tl-[85px] shadow-lg lg:shadow-2xl relative group">
                    <img src="{{ asset('images/therealcontent3aboutsupra.png') }}" alt="{{ __('Premium Vehicle Transport') }}"
                         class="w-full aspect-[16/9] object-cover transition-transform duration-1000 group-hover:scale-105">
                </div>
            </div>
        </div>
    ...
    <!-- TEXT CONTENT WRAPPER -->
    <div class="w-full lg:w-2/5 py-2 lg:py-8 px-1">
        <!-- Upgraded badge treatment for premium aesthetic pop -->
        <div class="inline-block px-3 py-1.5 bg-yellow-400 text-black text-[10px] font-black uppercase tracking-widest mb-3 rounded">
            {{ __('Elite Automotive Care') }}
        </div>

        <!-- Scaled down mobile header to prevent ugly line breaks -->
        <h3 class="text-xl sm:text-2xl lg:text-3xl font-black text-black leading-tight mb-3 transition-colors group-hover:text-yellow-600">
            {{ __('Next-Level Automotive Transit:') }}
        </h3>

        <!-- Consistent left border padding to match the standard blueprint layout perfectly -->
        <p class="text-gray-600 text-sm sm:text-base leading-relaxed mb-5 opacity-90 border-l-4 border-yellow-400 pl-4">
            {{ __("When it comes to moving elite automotive engineering, standard transport simply won't suffice.") }}
        </p>
...
<div class="group flex flex-col lg:flex-row-reverse items-center gap-6 lg:gap-24 relative"
    ...
    <!-- DELETED ON MOBILE: Giant absolute "04" hidden on mobile to prevent layout breaking -->
    <div class="hidden lg:block absolute -top-10 -right-55 text-[12rem] font-black text-black/[0.25] leading-none z-0 transition-all duration-700"
         :class="active ? '-translate-x-5' : ''">
        04
    </div>
    ...
    <div class="w-full lg:w-3/5 relative z-10">
        ...
        <div class="relative p-0.5 lg:p-2 border border-black/5 rounded-2xl lg:rounded-none lg:rounded-tr-[120px] lg:rounded-br-[120px] bg-white shadow-md lg:shadow-sm transition-transform duration-500"
            ...
            <div class="relative p-0 lg:p-4 lg:border-[16px] lg:border-yellow-400 lg:border-l-0 lg:rounded-none lg:rounded-tr-[110px] lg:rounded-br-[110px]">
                <div class="overflow-hidden rounded-xl lg:rounded-none lg:rounded-tr-[85px] lg:rounded-br-[85px] shadow-lg lg:shadow-2xl relative group">
                    <img src="{{ asset('images/therealcontent4redbull.jpeg') }}" alt="{{ __('Inter-state Cargo Consolidations') }}"
                         class="w-full aspect-[16/9] object-cover transition-transform duration-1000 group-hover:scale-105">
                </div>
            </div>
        </div>
    ...
    <div class="w-full lg:w-2/5 py-2 lg:py-8 px-1">
        <!-- Upgraded badge background & text padding contrast for visual pop -->
        <div class="inline-block px-3 py-1.5 bg-yellow-400 text-black text-[10px] font-black uppercase tracking-widest mb-3 rounded">
            {{ __('Mixed Cargo Solutions') }}
        </div>

        <!-- Optimized font size for mobile text scaling -->
        <h3 class="text-xl sm:text-2xl lg:text-3xl font-black text-black leading-tight mb-3 group-hover:text-yellow-600 transition-colors">
            {{ __('Recycling Project – Red Bull') }}
        </h3>

        <!-- Fixed border layout: Clean left-aligned accent border for perfect text flow on mobile and desktop -->
        <p class="text-gray-600 text-sm sm:text-base leading-relaxed mb-5 opacity-90 border-l-4 border-yellow-400 pl-4">
            {{ __('As part of this initiative, we were appointed to collect expired Red Bull stock from various locations throughout the region.') }}
        </p>
...
<div class="group flex flex-col lg:flex-row items-center gap-6 lg:gap-24 relative"
    ...
    <!-- DELETED ON MOBILE: Giant absolute "05" hidden on mobile to stop horizontal scroll bugs -->
    <div class="hidden lg:block absolute -top-10 -left-55 text-[12rem] font-black text-black/[0.25] leading-none z-0 transition-all duration-700"
         :class="active ? 'translate-x-5' : ''">
        05
    </div>
    ...
    <div class="w-full lg:w-3/5 relative z-10">
        ...
        <div class="relative p-0.5 lg:p-2 border border-black/5 rounded-2xl lg:rounded-none lg:rounded-tl-[120px] bg-white shadow-md lg:shadow-sm transition-transform duration-500"
            ...
            <div class="relative p-0 lg:p-4 lg:border-[16px] lg:border-yellow-400 lg:border-r-0 lg:border-b-0 lg:rounded-none lg:rounded-tl-[110px]">
                <div class="overflow-hidden rounded-xl lg:rounded-none lg:rounded-tl-[85px] shadow-lg lg:shadow-2xl relative group">
                    <img src="{{ asset('images/therealcontent5containers.jpeg') }}" alt="{{ __('Supply Chain Operations') }}"
                         class="w-full aspect-[16/9] object-cover transition-transform duration-1000 group-hover:scale-105">
                </div>
            </div>
        </div>
    ...
    <div class="w-full lg:w-2/5 py-2 lg:py-8 px-1">
        <!-- Upgraded badge treatment for premium aesthetic pop -->
        <div class="inline-block px-3 py-1.5 bg-yellow-400 text-black text-[10px] font-black uppercase tracking-widest mb-3 rounded">
            {{ __('Strategic Frameworks') }}
        </div>

        <!-- Scaled down mobile header to prevent ugly line breaks -->
        <h3 class="text-xl sm:text-2xl lg:text-3xl font-black text-black leading-tight mb-3 transition-colors group-hover:text-yellow-600">
            {{ __('Energizing The Digital Grid') }}
        </h3>

        <!-- Consistent left border padding to match the standard blueprint layout perfectly -->
        <p class="text-gray-600 text-sm sm:text-base leading-relaxed mb-5 opacity-90 border-l-4 border-yellow-400 pl-4">
            {{ __('When the continuity of digital enterprise hangs in the balance, transport precision is everything. Our strategic Kuala Lumpur to Kota Kinabalu deployment showcases Arkod Smart Logitech’s') }}
        </p>
    ...
    <div class="mt-40 p-12 bg-[#0a0a0a] rounded-[40px] relative overflow-hidden group shadow-2xl">
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
            <div>
                <h4 class="text-white text-3xl font-black uppercase tracking-tighter mb-2">{{ __('Ready to ship with the best?') }}</h4>
                <p class="text-gray-400 text-sm">{{ __('Join our list of successful partners and experience smart logistics.') }}</p>
            </div>
            <a href="#" class="bg-yellow-400 text-black px-10 py-5 font-black uppercase text-sm hover:bg-white transition-all transform hover:scale-105 shadow-xl">{{ __('Start Your Project') }}</a>
        </div>
    </div>
</div>
    ...
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" stroke="white"></path>
</svg>
            </button>

            <div class="w-full lg:w-1/2 bg-black relative min-h-[250px] lg:min-h-full">
                <img src="{{ asset('images/therealcontent1aboutpipes.jpeg') }}" alt="{{ __('Project Pipe Logistics') }}" class="w-full h-full object-cover absolute inset-0 opacity-90">
            ...
            <div class="w-full lg:w-1/2 p-8 sm:p-12 overflow-y-auto flex flex-col justify-between bg-white/40">
                <div>
                    <div class="flex items-center gap-3 mb-6">
                        <span class="px-2.5 py-1 bg-black text-white text-[10px] font-black uppercase tracking-widest">{{ __('Flash Report') }}</span>
                        <span class="text-xs text-gray-500 font-bold tracking-wider">{{ __('MANILA — KUCHING') }}</span>
                    </div>
                    ...
                    <div class="space-y-4 text-gray-800 text-base leading-relaxed font-medium">
                        <p class="text-lg font-bold text-gray-950">{{ __('From Manila to Kuching, we don’t just move cargo—we transport the massive infrastructure that shapes tomorrow.') }}</p>
                        <p>{!! __('Featured here is the seamless logistical execution of our monumental :size wall pipe shipment, precision-engineered and securely transported via specialized open-top containers.', ['size' => '<strong class="text-black">3500x3500x2000 mm</strong>']) !!}</p>
                        <p>{{ __('This critical milestone stands as a testament to our capacity for handling extreme-scale industrial demands, and it directly paves the way for our next major undertaking: anchoring the upcoming infrastructure development project in Serian, Sarawak.') }}</p>
                    </div>
                </div>
                <div class="mt-8 pt-6 border-t border-black/10 flex items-center justify-between text-xs text-gray-500 font-bold tracking-wider">
                    <span>{{ __('ARKOD SMART LOGITECH') }}</span>
                    <span>{{ __('SUCCESSFUL DELIVERY © 2026') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" stroke="white"></path>
            ...
            <div class="w-full lg:w-1/2 bg-black relative min-h-[250px] lg:min-h-full">
                <img src="{{ asset('images/therealcontent2aboutcar.png') }}" alt="{{ __('Car Shipping Details') }}" class="w-full h-full object-cover absolute inset-0 opacity-90">
                <div class="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r from-transparent via-transparent to-black/10"></div>
            </div>
           ...
           <div class="w-full lg:w-1/2 p-8 sm:p-12 overflow-y-auto flex flex-col justify-between bg-white/40">
                <div>
                    <div class="flex items-center gap-3 mb-6">
                        <span class="px-2.5 py-1 bg-black text-white text-[10px] font-black uppercase tracking-widest">{{ __('Arkod Smart Logitech Sdn. Bhd') }}</span>
                        <span class="text-xs text-gray-500 font-bold tracking-wider">{{ __('KUCHING — KUALA LUMPUR') }}</span>
                    </div>
                    <h2 class="text-3xl sm:text-4xl font-black text-black leading-tight mb-6 uppercase tracking-tight border-b-4 border-black pb-4">
                       {{ __('Bridging Oceans') }}, {{ __('Securing Valuables') }}
                    </h2>
                    <div class="space-y-4 text-gray-800 text-base leading-relaxed font-medium">
                        ...
                        <p>
                            {{ __('Featured here is our flawless end-to-end execution of a premium Toyota Camry transit from Kuching to Kuala Lumpur, securely bound alongside commercial batches of specialized palm sugar syrup.') }}
                        </p>
                        <p>
                            {{ __('Utilizing state-of-the-art blocking, bracing, and climate-defying insulation barriers within our specialized container units, we eliminate the risks of trans-sea relocation—proving once again that whether it is luxury engineering or bulk liquid commodities, your assets arrive in mint, pristine condition.') }}
                        </p>
                    </div>
                </div>
                <div class="mt-8 pt-6 border-t border-black/10 flex items-center justify-between text-xs text-gray-500 font-bold tracking-wider">
                    <span>{{ __('ARKOD SMART LOGITECH') }}</span>
                    <span>{{ __('SECURE TRANSIT © 2026') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" stroke="white"></path>
            ...
            <div class="w-full lg:w-1/2 bg-black relative min-h-[250px] lg:min-h-full">
                <img src="{{ asset('images/therealcontent3aboutsupra.png') }}" alt="{{ __('Furniture Delivery Details') }}" class="w-full h-full object-cover absolute inset-0 opacity-90">
                <div class="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r from-transparent via-transparent to-black/10"></div>
            </div>
             ...
             <div class="w-full lg:w-1/2 p-8 sm:p-12 overflow-y-auto flex flex-col justify-between bg-white/40">
                <div>
                    ...
                    <h2 class="text-3xl sm:text-4xl font-black text-black leading-tight mb-6 uppercase tracking-tight border-b-4 border-black pb-4">
                        {{ __('DOMINATING CROSS-BORDER HIGH-PERFORMANCE LOGISTICS') }} <br>
                    </h2>
                    <div class="space-y-4 text-gray-800 text-base leading-relaxed font-medium">
                        <p class="text-lg font-bold text-gray-950">
                            {{ __("When it comes to moving elite automotive engineering, standard transport simply won't suffice.") }}
                        ...
                        <p>
                            {{ __('Our latest successfully completed milestone showcases the flawless execution of a Full Container Load (FCL) transit, delivering a high-performance, used Toyota Supra from Kuching straight to Kuala Lumpur. From precision terminal handling to secure container tracking,') }}
                        </p>
                        <p>
                            {{ __('Arkod Smart Logitech utilizes rigid specialized rigging frameworks and custom bracing architectures designed to protect premium sports assets from the slightest cosmetic or structural shifting. We bridge the South China Sea with one absolute standard: zero compromise, total security, and pristine arrival condition.') }}
                        </p>
                    </div>
                </div>
                <div class="mt-8 pt-6 border-t border-black/10 flex items-center justify-between text-xs text-gray-500 font-bold tracking-wider">
    ...
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" stroke="white"></path>
            ...
            <div class="w-full lg:w-1/2 bg-black relative min-h-[250px] lg:min-h-full">
                <img src="{{ asset('images/therealcontent4redbull.jpeg') }}" alt="{{ __('Automotive Transit Details') }}" class="w-full h-full object-cover absolute inset-0 opacity-90">
                <div class="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r from-transparent via-transparent to-black/10"></div>
            </div>
            ...
            <div class="w-full lg:w-1/2 p-8 sm:p-12 overflow-y-auto flex flex-col justify-between bg-white/40">
                <div>
                    <div class="flex items-center gap-3 mb-6">
                        <span class="px-2.5 py-1 bg-black text-white text-[10px] font-black uppercase tracking-widest">{{ __('Arkod Smart Logitech Sdn. Bhd') }}</span>
                        <span class="text-xs text-gray-500 font-bold tracking-wider">{{ __('Recycling Project') }}</span>
                    </div>
                    <h2 class="text-3xl sm:text-4xl font-black text-black leading-tight mb-6 uppercase tracking-tight border-b-4 border-black pb-4">
                        {{ __('Recycling Project – Red Bull') }}
                    </h2>
                    <div class="space-y-4 text-gray-800 text-base leading-relaxed font-medium">
                        <p class="text-lg font-bold text-gray-950">
                            {{ __('In 2024, we successfully completed one of our largest logistics and supply distribution projects across Sarawak.') }}
                        </p>
                        <p>
                            {{ __('As part of this initiative, we were appointed to collect expired Red Bull stock from various locations throughout the region. The operation involved handling palletized shipments of Red Bull products, carefully managed and prepared for transport as part of the collection and recycling process.') }}
                        </p>
                        <p>
                            {{ __('Through careful planning, efficient logistics coordination, and reliable transportation management, we ensured timely delivery and collection across multiple locations while maintaining the highest standards of service.') }}
                        </p>
                    </div>
                </div>
                <div class="mt-8 pt-6 border-t border-black/10 flex items-center justify-between text-xs text-gray-500 font-bold tracking-wider">
    ...
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12" stroke="white"></path>
            ...
            <div class="w-full lg:w-1/2 bg-black relative min-h-[250px] lg:min-h-full">
                <img src="{{ asset('images/therealcontent5containers.jpeg') }}" alt="{{ __('Infrastructure Project Details') }}" class="w-full h-full object-cover absolute inset-0 opacity-90">
                <div class="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r from-transparent via-transparent to-black/10"></div>
            </div>
            ...
            <div class="w-full lg:w-1/2 p-8 sm:p-12 overflow-y-auto flex flex-col justify-between bg-white/40">
                <div>
                    <div class="flex items-center gap-3 mb-6">
                        <span class="px-2.5 py-1 bg-black text-white text-[10px] font-black uppercase tracking-widest">{{ __('Arkod Smart Logitech Sdn. Bhd') }}</span>
                        <span class="text-xs text-gray-500 font-bold tracking-wider">{{ __('KUALA LUMPUR - KOTA KINABALU') }}</span>
                    </div>
                    <h2 class="text-3xl sm:text-4xl font-black text-black leading-tight mb-6 uppercase tracking-tight border-b-4 border-black pb-4">
                        {{ __('UNCOMPROMISING LOGISTICS FOR CRITICAL TECHNOLOGY') }}
                    </h2>
                    <div class="space-y-4 text-gray-800 text-base leading-relaxed font-medium">
                        <p class="text-lg font-bold text-gray-950">{{ __('Our strategic Kuala Lumpur to Kota Kinabalu deployment showcases Arkod Smart Logitech’s capacity to securely handle dense, high-sensitivity technological infrastructure') }}</p>
                        <p>
                            {{ __('Safely distributing complete industrial UPS machines, high-capacity battery banks, and heavy server cabinet racks. Packed with advanced weather-sealed moisture barriers, industrial pallet anchoring systems, and heavy-duty poly-strapping configurations,') }}
                        </p>
                        <p>
                            {{ __('We safeguard delicate internal electronic components against complex sea-route vibrations and humidity. From tech hubs to regional data centers, we deliver the power that keeps East Malaysia running.') }}
                        </p>
                    </div>
                </div>
                <div class="mt-8 pt-6 border-t border-black/10 flex items-center justify-between text-xs text-gray-500 font-bold tracking-wider">
        ...
        slides: [
            {
                ...
                title: "{{ __('PROVEN LOGISTICS') }}",
                subtitle: "{{ __('Excellence in Motion') }}",
                description: "{{ __('Experience the efficiency of our completed shipments and projects.') }}",
                kpis: [
                    { icon: '🚛', number: "{{ __('VARIOUS') }}", label: "{{ __('Belongings Shipped') }}" },
                    { icon: '⚡', number: "{{ __('EFFICIENT') }}", label: "{{ __('Handling Management') }}" },
                    { icon: '🤝', number: "{{ __('PARTNERED') }}", label: "{{ __('Various Companies') }}" }
                ]
            },

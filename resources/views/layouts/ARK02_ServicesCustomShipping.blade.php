...
<body class="bg-[#0a0a0a] font-montserrat antialiased text-white">
    ...
    <section class="relative min-h-[10vh] md:min-h-screen w-full flex items-center overflow-hidden -mt-[72px] md:-mt-[88px]" x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 100)">
        ...
        <div class="absolute inset-0 z-0">
            <img src="{{ asset('images/CUSTOM SHIPPING 2.png') }}" alt="{{ __('ARKOD Logistics') }}" class="w-full h-full object-cover transition-all duration-[4000ms] ease-out" :class="loaded ? 'opacity-70 blur-0 scale-105' : 'opacity-0 blur-xl scale-125'">
        ...
        <div class="relative z-20 max-w-[1600px] mx-auto px-6 md:px-8 w-full mt-10 md:mt-20">
            <div class="max-w-8xl text-center lg:text-left">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-8 md:gap-10 transition-all duration-1000 delay-700" :class="loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'">
                    <a href="#custom-handling"
                    style="scroll-behavior: smooth;"
                    class="w-full sm:w-auto text-center relative overflow-hidden bg-yellow-400 hover:bg-white text-black px-12 py-5 md:px-16 md:py-6 rounded-sm text-lg md:text-lg font-black uppercase tracking-[0.3em] transition-all duration-500 group shadow-lg">
                   ...
                   <a href="https://wa.me/60189116168?text=Hi%20ARKOD%2C%20I%20am%20interested%20in%20your%20custom%20shipping%20and%20logistics%20services."
                    target="_blank"
                    rel="noopener noreferrer"
                    class="flex items-center gap-4 group cursor-pointer">
                        <div class="w-12 h-12 md:w-14 md:h-14 rounded-full border border-white/20 flex items-center justify-center group-hover:bg-yellow-400 group-hover:border-yellow-400 transition-all duration-500 shrink-0">
                            <svg class="w-4 h-4 text-white group-hover:text-black fill-current ml-1" viewBox="0 0 24 24">
    ...
    <section id="custom-handling" class="py-20 bg-white text-[#0a0a0a]">
        <div class="max-w-[1600px] mx-auto px-6 md:px-8">
            <div class="grid lg:grid-cols-12 gap-16 items-center">

                <!-- ==================== RESPONSIVE INTRO TEXT COLUMN ==================== -->
<div class="lg:col-span-5 text-center lg:text-left px-2 sm:px-0">
    <!-- Centered badge layout on mobile viewports -->
    <div class="inline-flex items-center gap-2 px-5 py-2 border border-yellow-400/50 rounded-full bg-yellow-400/10 mb-6 mx-auto lg:mx-0">
        <span class="text-yellow-400 text-xs sm:text-sm font-black uppercase tracking-widest">{{ __('WHAT WE DO') }}</span>
    </div>

    <!-- Fluid typography size adaptation -->
    <h2 class="text-3xl sm:text-4xl md:text-5xl font-black uppercase leading-tight tracking-tighter mb-6 md:mb-8 text-black">
        {{ __('Beyond Standard,') }}<br>
        <span class="text-yellow-400">{{ __('Shaping Your Logistics') }}</span>
    </h2>

    <!-- Dynamic alignment for decorative horizontal rule element -->
    <div class="h-px w-24 bg-yellow-400 mb-6 md:mb-8 mx-auto lg:mx-0"></div>

    <!-- Comfort text read sizing wrapper -->
    <p class="text-gray-700 text-base md:text-lg leading-relaxed max-w-2xl mx-auto lg:mx-0">
        {{ __('We know standard freight doesn’t fit unique requirements. By collaborating closely from the very first agreement, we establish specialized handling protocols and custom shipping terms that ensure premium, precise execution on every project.') }}
    </p>
</div>
